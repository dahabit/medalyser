	...
	public function submitformAction() {
		...
		if ($this->getRequest ()->isPost ()) {
			...
			if ($form->isValid ( $this->_request->getPost () )) {
				...
				$profilephoto->setDestination ( PUBLIC_PATH . '\images\patient\profile' );
				...
				if (! empty ( $primaryemail )) {
					...
					$account = $this->em->getRepository ( 'Entities\Patientprofile' )->findOneBy ( array ('primaryemail' => $this->_request->getParam ( 'primaryemail' ) ) );
				...
				if (! isset ( $account )) {
					...
					$account->setFirstname ( $this->_request->getParam ( 'firstname' ) );
					$account->setMiddlename ( $this->_request->getParam ( 'middlename' ) );
					$account->setLastname ( $this->_request->getParam ( 'lastname' ) );
					
					//convert date and time to object so doctrine doesn't echo errors
					$account->setBirthdate ( $this->_request->getParam ( 'birthdate' ) );
					$account->setMaritalstatus ( $this->_request->getParam ( 'maritalstatus' ) );
					$account->setPrimaryemail ( $this->_request->getParam ( 'primaryemail' ) );
					$account->setSex ( $this->_request->getParam ( 'sex' ) );
					$account->setSocialsecurity ( $this->_request->getParam ( 'socialsecurity' ) );
					...
					// Addresses
					$addressType = $this->_request->getParam ( 'addresstype' );
					...
					// Phone numbers
					$account->setHomephone ( $this->_request->getParam ( 'homephone' ) );
					$account->setBusinessphone ( $this->_request->getParam ( 'businessphone' ) );
					$account->setMobilephone ( $this->_request->getParam ( 'mobilephone' ) );
					$account->setFaxphone ( $this->_request->getParam ( 'faxphone' ) );
					/////////////////////end of PAGE 2/////////////////////
					...
					try {
						...
						$form = array ('success' => true, 'msg' => 'New patient successfully created' );
						$this->_response->appendBody ( Zend_Json::encode ( $form ) );
					
					} catch ( Exception $e ) {
						
						$error = array ('success' => false, 'msg' => 'Error saving data to the database.Please contact administrator.' );
						$this->_response->appendBody ( Zend_Json::encode ( $error ) );
						
						//log exception to firebug
						$this->_response->appendBody ( Zend_Json::encode ( $error ) );
						Zend_Registry::get ( 'logger' )->crit ( 'Caught exception: ' . $e->getMessage () );
						var_dump ( $e->getMessage () );
					}
				
				} else {
					
					$this->_response->appendBody ( Zend_Json::encode ( "The desired useremail has already been taken, or
              the provided e-mail address is already associated with a registered user." ) );
				...
				Zend_Registry::get ( 'logger' )->crit ( 'validation was a failure' );
				//Output validation error messagaes as json format
				$validationerror = array ('success' => false, 'msg' => 'Some entered data are not acceptable. Please check back, correct them and try submitting the form again.', $form->getMessages () );
