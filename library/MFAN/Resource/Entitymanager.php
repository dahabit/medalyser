 ...
 *
 */
class MFAN_Resource_Entitymanager extends Zend_Application_Resource_ResourceAbstract
{
    public function init ()
    {
        ...
        $options = array(
        'connection' => array('driver' => "{$config->connection->driver}", 
        ...
        $configEm->setProxyDir($config->connection->proxies->location);
        // Configure cache
        $configEm->setQueryCacheImpl($cache);
        $em = \Doctrine\ORM\EntityManager::create($options['connection'], 
        $configEm);
