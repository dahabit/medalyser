...
class SchemaException extends \Doctrine\DBAL\DBALException
{
    ...
    {
        return new self("Index '$indexName' does not exist on table '$table'.", 
    ...
    {
        return new self("The table with name '" . $tableName .
         "' already exists.", self::TABLE_ALREADY_EXISTS);
    }
    /**
     *
     * @param string $tableName
    ...
    ForeignKeyConstraint $foreignKey)
    {
        ...
        "The performed schema operation on " . $localTable->getName() .
         " requires a named foreign key, " . "but the given foreign key from (" .
         implode(", ", $foreignKey->getColumns()) . ") onto foreign table " . "'" .
         $foreignKey->getForeignTableName() . "' (" .
         implode(", ", $foreignKey->getForeignColumns()) . ") is currently " .
         "unnamed.");
    }
