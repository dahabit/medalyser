...
class InputDefinition
{
    ...
    public function addOption (InputOption $option)
    {
        if (isset($this->options[$option->getName()])) {
            throw new \LogicException(
            sprintf('An option named "%s" already exist.', $option->getName()));
        } else 
    ...
    public function getSynopsis ()
    {
        ...
        foreach ($this->getOptions() as $option) {
            ...
            $elements[] = sprintf(
            '[' .
             ($option->isValueRequired() ? '%s--%s="..."' : ($option->isValueOptional() ? '%s--%s[="..."]' : '%s--%s')) .
    ...
    public function asText ()
    {
        ...
        $max = 0;
        foreach ($this->getOptions() as $option) {
            $max = strlen($option->getName()) + 2 > $max ? strlen(
            $option->getName()) + 2 : $max;
        }
        foreach ($this->getArguments() as $argument) {
            $max = strlen($argument->getName()) > $max ? strlen(
            $argument->getName()) : $max;
        }
        ++ $max;
        $text = array();
        if ($this->getArguments()) {
            ...
            foreach ($this->getArguments() as $argument) {
                ...
                $argument->getName(), $argument->getDescription(), $default);
            }
            $text[] = '';
        }
        if ($this->getOptions()) {
            ...
            foreach ($this->getOptions() as $option) {
                ...
                $text[] = sprintf(' %-' . $max . 's %s%s%s%s', 
                '<info>--' . $option->getName() . '</info>', 
                $option->getShortcut() ? sprintf('(-%s) ', 
                $option->getShortcut()) : '', $option->getDescription(), $default, 
                $multiple);
            }
            $text[] = '';
        }
    ...
    public function asXml ($asDom = false)
    {
        ...
        foreach ($this->getArguments() as $argument) {
            ...
            $defaults = is_array($argument->getDefault()) ? $argument->getDefault() : ($argument->getDefault() ? array(
        ...
        foreach ($this->getOptions() as $option) {
            ...
            if ($option->acceptValue()) {
                $optionXML->appendChild(
                $defaultsXML = $dom->createElement('defaults'));
                $defaults = is_array($option->getDefault()) ? $option->getDefault() : ($option->getDefault() ? array(
