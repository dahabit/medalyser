...
Zend_Mail_Storage_Writable_Interface
{
    ...
    public function createFolder ($name, $parentFolder = null)
    {
        ...
        if (strpos($folder, $this->_delim)) {
            ...
            $parent = substr($folder, 0, 
            strrpos($folder, $this->_delim));
            try {
    ...
    public function removeFolder ($name)
    {
        ...
        if (! rmdir($this->_rootdir . '.' . $name)) {
            ...
            mkdir(
            $this->_rootdir . '.' . $name . DIRECTORY_SEPARATOR . 'cur');
            /**
             * @see Zend_Mail_Storage_Exception
             */
            require_once 'Zend/Mail/Storage/Exception.php';
    ...
    protected function _createUniqueId ()
    {
        ...
        $id .= function_exists('microtime') ? microtime(true) : (time() . ' ' .
         rand(0, 100000));
        $id .= '.' .
         (function_exists('posix_getpid') ? posix_getpid() : rand(50, 65535));
        $id .= '.' . php_uname('n');
        return $id;
    }
    /**
    ...
    protected function _createTmpFile ($folder = 'INBOX')
    {
        ...
        'uniq' => $uniq, 'filename' => $tmpdir . $uniq, 'handle' => $fh);
    }
    /**
    ...
    $recent = false)
    {
        ...
        'flags' => $flags, 'filename' => $new_filename);
        if ($this->_quota) {
    ...
    public function copyMessage ($id, $folder)
    {
        ...
        while (($key = array_search(Zend_Mail_Storage::FLAG_RECENT, 
        $flags)) !== false) {
            unset($flags[$key]);
        }
        $info = $this->_getInfoString($flags);
    ...
    protected function _calculateMaildirsize ()
    {
        ...
        return array('size' => $total_size, 'count' => $messages, 
        'quota' => $quota);
    }
    /**
    ...
    protected function _calculateQuota ($forceRecalc = false)
    {
        ...
        return array('size' => $total_size, 'count' => $messages, 
        'quota' => $quota, 'over_quota' => $over_quota);
    }
