...
class Zend_Queue_Adapter_Db extends Zend_Queue_Adapter_AdapterAbstract
{
    ...
    protected function _initDbAdapter ()
    {
        ...
        try {
            ...
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception(
            'Error connecting to database: ' . $e->getMessage(), $e->getCode(), 
            $e);
        }
        return $db;
    }
    ...
    public function count (Zend_Queue $queue = null)
    {
        ...
        $query = $db->select();
        $query->from($info['name'], array(new Zend_Db_Expr('COUNT(1)')))->where(
    ...
    Zend_Queue $queue = null)
    {
        ...
        try {
            if ($maxMessages > 0) { // ZF-7666 LIMIT 0 clause not included.
                ...
                foreach ($db->fetchAll($query) as $data) {
                    ...
                    $data['handle'] = md5(
                    uniqid(rand(), true));
                    $update = array('handle' => $data['handle'], 
                    'timeout' => $microtime);
                    // update the database
                    $where = array();
        ...
        $options = array('queue' => $queue, 'data' => $msgs, 
    ...
    {
        return array('create' => true, 'delete' => true, 
        'send' => true, 'receive' => true, 'deleteMessage' => true, 
        'getQueues' => true, 'count' => true, 'isExists' => true);
    }
    /********************************************************************
    ...
    protected function getQueueId ($name)
    {
        ...
        $query = $this->_queueTable->select();
        $query->from($this->_queueTable, array('queue_id'))->where(
        'queue_name=?', $name);
