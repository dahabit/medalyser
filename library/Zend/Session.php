...
class Zend_Session extends Zend_Session_Abstract
{
     ...
     * @var array
     */
    private static $_defaultOptions = array('save_path' => null, 
    ...
    'gc_probability' => null, 
    'gc_divisor' => null, 'gc_maxlifetime' => null, 
    'serialize_handler' => null, 'cookie_lifetime' => null, 
    'cookie_path' => null, 'cookie_domain' => null, 
    'cookie_secure' => null, 'cookie_httponly' => null, 
    'use_cookies' => null, 'use_only_cookies' => 'on', 
    'referer_check' => null, 'entropy_file' => null, 
    'entropy_length' => null, 'cache_limiter' => null, 
    'cache_expire' => null, 'use_trans_sid' => null, 
    'bug_compat_42' => null, 'bug_compat_warn' => null, 
    'hash_function' => null, 'hash_bits_per_character' => null);
    /**
     ...
     * @var array
     */
    private static $_localOptions = array('strict' => '_strict', 
    ...
    public static function setOptions (array $userOptions = array())
    {
        ...
        foreach ($userOptions as $userOptionName => $userOptionValue) {
            ...
            if (array_key_exists($userOptionName, 
    ...
    Zend_Session_SaveHandler_Interface $saveHandler)
    {
        ...
        session_set_save_handler(array(&$saveHandler, 'open'), 
        array(&$saveHandler, 'close'), array(&$saveHandler, 'read'), 
        array(&$saveHandler, 'write'), array(&$saveHandler, 'destroy'), 
        ...
        if (! self::$_unitTestEnabled) {
            ...
            if (! $startedCleanly ||
             Zend_Session_Exception::$sessionStartError != null) {
                if (self::$_throwStartupExceptions) {
                    set_error_handler(
                    array('Zend_Session_Exception', 'handleSilentWriteClose'), 
                    $errorLevel);
                }
                session_write_close();
                if (self::$_throwStartupExceptions) {
...
$cookie_params['path'], 
            $cookie_params['domain'], $cookie_params['secure']);
        }
    }
    /**
