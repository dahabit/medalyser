...
Zend_Http_UserAgent_Device
{
    ...
    {
        $spec = array('_aFeatures' => $this->_aFeatures, 
        '_aGroup' => $this->_aGroup, '_browser' => $this->_browser, 
        '_browserVersion' => $this->_browserVersion, 
        '_userAgent' => $this->_userAgent, '_images' => $this->_images);
    ...
    protected function _getDefaultFeatures ()
    {
        ...
        if (isset($this->_server['server_software'])) {
            if (strpos($this->_server['server_software'], 'Apache') !== false ||
             strpos($this->_server['server_software'], 'LiteSpeed') !== false) {
                $server['version'] = 1;
    ...
    public static function extractFromUserAgent ($userAgent)
    {
        ...
        if ($match2) {
            ...
            for ($i = 0; $i < $max; $i ++) {
                if (! empty($match2[0][$i])) {
                    $result['others']['detail'][] = array($match2[0][$i], 
                    $match2[2][$i], $match2[4][$i]);
                }
            }
        }
        /** Security level */
        $security = array('N' => 'no security', 'U' => 'strong security', 
        'I' => 'weak security');
        ...
        if (isset($result['device_os_token'])) {
            if (strpos($result['device_os_token'], 'Win') !== false) {
                $windows = array('Windows NT 6.1' => 'Windows 7', 
        ...
        if (isset($result['others'])) {
            if ($result['others']['detail'][0][1] == 'AppleWebKit') {
                ...
                if (strpos($result['browser_version'], '.') > 2 ||
                 (int) $result['browser_version'] > 20) {
                    ...
                    $build = (int) $temp[0];
                    $awkVersion = array(48 => '0.8', 73 => '0.9', 85 => '1.0', 
                    103 => '1.1', 124 => '1.2', 300 => '1.3', 400 => '2.0');
            ...
            if ($result['others']['detail'][0][1] == 'Gecko') {
                ...
                if (! empty($result['others']['detail'][1][1]) &&
                 ! empty(
                $result['others']['detail'][count($result['others']['detail']) -
                 1][2]) ||
                 strpos(strtolower($result['others']['full']), 'opera') !== false) {
                    ...
                    if ($result['browser_name'] ==
                     'Navigator' || $result['browser_name'] == 'Netscape6') {
                        $result['browser_name'] = 'Netscape';
                    }
                }
                if ($searchRV) {
        ...
        // UA ends with 'Opera X.XX'
        if (isset($result['browser_name']) &&
         isset($result['browser_engine'])) {
        ...
        // cleanup
        if (isset($result['browser_version']) &&
         isset($result['browser_build'])) {
    ...
    protected function _loadFeaturesAdapter ()
    {
        ...
        if (! isset($config[$browserType]) ||
         ! isset($config[$browserType]['features'])) {
            return array();
        }
        ...
        return call_user_func(array($className, 'getFromRequest'), 
     ...
     * @return bool
     */
    protected static function _matchAgentAgainstSignatures ($userAgent, 
    $signatures)
    {
