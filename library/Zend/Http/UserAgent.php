...
class Zend_Http_UserAgent implements Serializable
{
    ...
    public function serialize ()
    {
        ...
        $spec = array('browser_type' => $this->_browserType, 
        'config' => $this->_config, 'device_class' => get_class($device), 
    ...
    protected function _match ($deviceClass)
    {
        ...
        return call_user_func(array($deviceClass, 'match'), $userAgent, 
        $this->getServer());
    }
    /**
    ...
    public function setPluginLoader ($type, $loader)
    {
        ...
        if (is_string($loader)) {
            ...
            require_once 'Zend/Http/UserAgent/Exception.php';
            throw new Zend_Http_UserAgent_Exception(
            sprintf('Expected a plugin loader class or object; received %s', 
            gettype($loader)));
        }
    ...
    protected function _validateLoaderType ($type)
    {
        ...
        if (! in_array($type, $this->_loaderTypes)) {
            ...
            throw new Zend_Http_UserAgent_Exception(
            sprintf(
            'Expected one of "%s" for plugin loader type; received "%s"', $types, 
            (string) $type));
        }
        return $type;
    }
    ...
    protected function _matchUserAgent ()
    {
        ...
        $sequence = explode(',', 
        $this->_config['identification_sequence']);
