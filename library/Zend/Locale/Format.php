...
class Zend_Locale_Format
{
    ...
    public static function getNumber ($input, array $options = array())
    {
        ...
        $symbols = Zend_Locale_Data::getList($options['locale'], 
        'symbols');
    ...
    public static function toNumber ($value, array $options = array())
    {
        ...
        // set negative sign
        if (call_user_func(Zend_Locale_Math::$comp, $value, 0, 
        $options['precision']) < 0) {
    ...
    {
        if (iconv_strpos($format, ';') !== false) {
            if (call_user_func(Zend_Locale_Math::$comp, $value, 0, $precision) <
             0) {
    ...
    public static function isNumber ($input, array $options = array())
    {
        ...
        $symbols = Zend_Locale_Data::getList($options['locale'], 
        'symbols');
        $regexs = Zend_Locale_Format::_getRegexForType('decimalnumber', 
        $options);
    ...
    private static function _parseDate ($date, $options)
    {
        ...
        if ($day !== false) {
            ...
            if (! empty($options['locale']) && ($options['locale'] !== 'root') &&
             ...
             ((string) $options['locale'] !== 'root'))) {
                // erase day string
                $daylist = Zend_Locale_Data::getList(
                $options['locale'], 'day');
        ...
        if (iconv_strpos($format, 'a') !== false) {
            ...
            strtoupper(Zend_Locale_Data::getContent($options['locale'], 'am'))) !==
             false) {
                $am = true;
            } else 
                if (iconv_strpos(strtoupper($number), 
                strtoupper(
                Zend_Locale_Data::getContent($options['locale'], 'pm'))) !== false) {
                    $am = false;
                }
        }
        // split number parts
