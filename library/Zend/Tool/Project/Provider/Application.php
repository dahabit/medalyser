...
Zend_Tool_Framework_Provider_Pretendable
{
    ...
    public function changeClassNamePrefix ($classNamePrefix /* , $force = false */)
    {
        ...
        if ($zc->appnamespace == $classNamePrefix) {
            ...
            'The requested name ' . $classNamePrefix . ' is already the prefix.');
        }
        // remove the old
        $configFileResource->removeStringItem('appnamespace', 
        'production');
        $configFileResource->create();
        // add the new
        $configFileResource->addStringItem('appnamespace', 
        $classNamePrefix, 'production', true);
        $configFileResource->create();
        ...
        array('color' => 'yellow'));
        $response->appendContent(
        'application.ini updated with new appnamespace ' . $classNamePrefix);
        // store profile
        $this->_storeProfile();
    }
}
