...
class Zend_Tool_Framework_Client_Console_HelpSystem
{
    ...
    Exception $exception = null)
    {
        ...
        array('color' => array('hiWhite', 'bgRed'), 'aligncenter' => true));
        $this->_response->appendContent($errorMessage, 
        array('indention' => 1, 'blockize' => 72, 
        'color' => array('white', 'bgRed')));
    ...
    public function respondWithGeneralHelp ()
    {
        ...
        $this->_response->appendContent('Usage:', array('color' => 'green'))
            ...
            ->appendContent('zf', 
        array_merge(array('color' => 'cyan'), $noSeparator))
    ...
    protected function _respondWithHeader ()
    {
        ...
        array('color' => array('hiWhite'), 'separator' => false));
        $this->_response->appendContent(
    ...
    $actionNameFilter = null, $includeAllSpecialties = false)
    {
        ...
        $providerMetadatasSearch = array('type' => 'Tool', 
        'name' => 'providerName', 'clientName' => 'console');
        ...
        $actionMetadatasSearch = array('type' => 'Tool', 
        'name' => 'actionName', 'clientName' => 'console');
        if (is_string($actionNameFilter)) {
            $actionMetadatasSearch = array_merge($actionMetadatasSearch, 
            array('actionName' => $actionNameFilter));
        }
        ...
        foreach ($displayProviderMetadatas as $providerMetadata) {
            ...
            foreach ($providerSignature->getActions() as $actionInfo) {
                ...
                if (($foundActionIndex = array_search(
                $actionName, $displayActionNames)) === false) {
                    continue;
                } else {
                    $actionMetadata = $displayActionMetadatas[$foundActionIndex];
                }
                $specialtyMetadata = $manifest->getMetadata(
                array('type' => 'Tool', 'name' => 'specialtyName', 
                'providerName' => $providerName, 'specialtyName' => '_Global', 
                'clientName' => 'console'));
                ...
                'providerName' => $providerName, 'specialtyName' => '_Global', 
                'actionName' => $actionName, 'clientName' => 'console'));
                $actionableGlobalMetadatas = $manifest->getMetadatas(
                array('type' => 'Tool', 
                'name' => 'actionableMethodLongParams', 
                'providerName' => $providerName, 'actionName' => $actionName, 
                'clientName' => 'console'));
                ...
                if (! $includeAllSpecialties &&
                 count($actionableGlobalMetadatas) > 1) {
                    ...
                    $this->_response->appendContent(
                    ' to get specific help on them.', array('color' => 'yellow'));
                }
            }
            if ($providerNameDisplayed) {
