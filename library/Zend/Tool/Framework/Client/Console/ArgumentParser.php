...
Zend_Tool_Framework_Registry_EnabledInterface
{
    ...
    public function parse ()
    {
        ...
        if (count($this->_argumentsWorking) == 0) {
            ...
            if ($this->_help) {
                ...
                $this->_createHelpResponse(
                array('error' => 'An action and provider is required.'));
            }
            return;
        }
        ...
        if (count($this->_argumentsWorking) != 0) {
            ...
            if ($this->_help) {
                ...
                'error' => 'Unknown arguments left on the command line: ' .
                 implode(' ', $this->_argumentsWorking))));
            }
            return;
        }
    ...
    protected function _parseGlobalPart ()
    {
        ...
        $this->_argumentsWorking, array('parseAll' => false));
    ...
    protected function _parseActionPart ()
    {
        ...
        $actionSearchCriteria = array('type' => 'Tool', 
        'name' => 'actionName', 'value' => $consoleActionName, 
        'clientName' => 'console');
        ...
        if (! $actionMetadata) {
            ...
            str_replace(array('-', '_'), '', $consoleActionName));
    ...
    protected function _parseProviderPart ()
    {
        ...
        $providerSearchCriteria = array('type' => 'Tool', 
        'name' => 'providerName', 'value' => $consoleProviderName, 
        'clientName' => 'console');
        ...
        if (! $providerMetadata) {
            ...
            str_replace(array('-', '_'), '', $consoleProviderName));
        ...
        $providerSpecialtySearchCriteria = array('type' => 'Tool', 
        'name' => 'specialtyName', 'value' => $consoleSpecialtyName, 
    ...
    protected function _parseProviderOptionsPart ()
    {
        ...
        array('name' => 'actionableMethodLongParams')));
        $actionableMethodShortParamsMetadata = $this->_manifestRepository->getMetadata(
        array_merge($searchParams, 
        array('name' => 'actionableMethodShortParams')));
        ...
        foreach ($actionableMethodLongParamsMetadata->getValue() as $parameterNameLong => $consoleParameterNameLong) {
            ...
            } elseif (in_array($parameterInfo['type'], 
            array('int', 'integer', 'float'))) {
                $optionConfig .= $paramNameShortValues[$parameterNameLong] .
        ...
        while (($wordOnTop = array_shift($this->_argumentsWorking))) {
            if (substr($wordOnTop, 0, 1) != '-') {
                ...
                array_unshift($this->_argumentsWorking, 
                $wordOnTop);
                break;
            }
    ...
    protected function _createHelpResponse ($options = array())
    {
        ...
        if (isset($options['actionName']) && isset($options['providerName'])) {
            $helpSystem->respondWithSpecialtyAndParamHelp(
            $options['providerName'], $options['actionName']);
