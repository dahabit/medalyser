...
class Zend_Pdf
{
        ...
        foreach ($pages->Kids->items as $child) {
            if ($child->Type->value == 'Pages') {
                ...
                if ($child->Type->value == 'Page') {
                    ...
                    require_once 'Zend/Pdf/Page.php';
                    $this->pages[] = new Zend_Pdf_Page($child, 
                    $this->_objFactory);
                }
        }
    }
            ...
            if ($root->Names !== null &&
             $root->Names->Dests !== null) {
                ...
                foreach (new Zend_Pdf_NameTree($root->Names->Dests) as $name => $destination) {
                    $this->_namedTargets[$name] = Zend_Pdf_Target::load(
                    $destination);
                }
            }
        } else {
    ...
    protected function _loadOutlines (Zend_Pdf_Element_Reference $root)
    {
        ...
        if ($root->Outlines->Type !== null &&
         $root->Outlines->Type->value != 'Outlines') {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(
    ...
    protected function _dumpPages ()
    {
        ...
        foreach ($this->_namedTargets as $name => $namedTarget) {
            if ($namedTarget instanceof Zend_Pdf_Destination_Explicit) {
                ...
                if ($this->resolveDestination($namedTarget, 
                false) === null) {
                ...
                if ($namedTarget instanceof Zend_Pdf_Action) {
                    ...
                    if ($this->_cleanUpAction($namedTarget, 
                    false) === null) {
                        // Action is a GoTo action with an unresolved destination
                        unset(
                        $this->_namedTargets[$name]);
                    }
                } else {
                    require_once 'Zend/Pdf/Exception.php';
                    throw new Zend_Pdf_Exception(
        ...
        foreach ($iterator as $outline) {
            ...
            if ($target !== null) {
                if ($target instanceof Zend_Pdf_Destination) {
                    ...
                    if ($this->resolveDestination($target, 
                    false) === null) {
        ...
        if ($openAction !== null) {
            if ($openAction instanceof Zend_Pdf_Action) {
                ...
                if ($this->_cleanUpAction($openAction, false) ===
                 null) {
                    // Action is a GoTo action with an unresolved destination
    ...
    protected function _dumpNamedDestinations ()
    {
        ...
        $DestTree = $this->_objFactory->newObject(
        new Zend_Pdf_Element_Dictionary());
    ...
    protected function _dumpOutlines ()
    {
        ...
        if ($updateOutlinesNavigation) {
            ...
            foreach ($this->outlines as $outline) {
                if ($lastOutline === null) {
                    ...
                    $lastOutline = $outline->dumpOutline(
        ...
        if ($openOutlinesCount != $this->_originalOpenOutlinesCount) {
            $root->Outlines->touch;
            $root->Outlines->Count = new Zend_Pdf_Element_Numeric(
            $openOutlinesCount);
        }
    }
    /**
    ...
    public function extractFonts ()
    {
        ...
        foreach ($this->pages as $page) {
            ...
            foreach ($fontResources->getKeys() as $fontResourceName) {
                ...
                $fontResourcesUnique[spl_object_hash(
                $fontDictionary->getObject())] = $fontDictionary;
            }
        }
        $fonts = array();
        require_once 'Zend/Pdf/Exception.php';
    ...
    public function render ($newSegmentOnly = false, $outputStream = null)
    {
        ...
        if ($this->properties != $this->_originalProperties) {
            $docInfo = $this->_objFactory->newObject(
            new Zend_Pdf_Element_Dictionary());
            foreach ($this->properties as $key => $value) {
                switch ($key) {
                    case 'Trapped':
                        switch ($value) {
                            case true:
                                $docInfo->$key = new Zend_Pdf_Element_Name(
                                'True');
                                break;
                            case false:
                                $docInfo->$key = new Zend_Pdf_Element_Name(
                    ...
                    case 'ModDate':
                        $docInfo->$key = new Zend_Pdf_Element_String(
                        (string) $value);
                        break;
                    case 'Title':
                    // break intentionally omitted
                    ...
                    default:
                        ...
                        $docInfo->$key = Zend_Pdf_Element::phpToPdf(
                        $value);
                        break;
                }
            }
