...
class Zend_Feed_Rss extends Zend_Feed_Abstract
{
    ...
    protected function _mapFeedHeaders ($array)
    {
        ...
        if (isset($array->webmaster)) {
            $channel->appendChild(
            $this->_element->createElement('webMaster', $array->webmaster));
        }
        ...
        if (isset($array->textInput)) {
            ...
            $textinput->appendChild(
            $this->_element->createElement('name', $array->textInput['name']));
            $textinput->appendChild(
            $this->_element->createElement('link', $array->textInput['link']));
    ...
    private function _buildiTunes (DOMElement $root, $array)
    {
        ...
        if (! empty($author)) {
            ...
            'http://www.itunes.com/DTDs/Podcast-1.0.dtd', 'itunes:author', 
            $author);
        ...
        if (! empty($author) || ! empty($email)) {
            ...
            if (! empty($author)) {
                $node = $this->_element->createElementNS(
                'http://www.itunes.com/DTDs/Podcast-1.0.dtd', 'itunes:name', 
                $author);
                $owner->appendChild($node);
            }
            if (! empty($email)) {
                $node = $this->_element->createElementNS(
                'http://www.itunes.com/DTDs/Podcast-1.0.dtd', 'itunes:email', 
                $email);
                $owner->appendChild($node);
            }
        ...
        if (! empty($subtitle)) {
            $node = $this->_element->createElementNS(
            'http://www.itunes.com/DTDs/Podcast-1.0.dtd', 'itunes:subtitle', 
            $subtitle);
            $root->appendChild($node);
        }
        $summary = '';
        ...
        if (! empty($summary)) {
            $node = $this->_element->createElementNS(
            'http://www.itunes.com/DTDs/Podcast-1.0.dtd', 'itunes:summary', 
            $summary);
            $root->appendChild($node);
        }
        ...
        if (isset($array->itunes->explicit)) {
            $node = $this->_element->createElementNS(
            'http://www.itunes.com/DTDs/Podcast-1.0.dtd', 'itunes:explicit', 
            $array->itunes->explicit);
            $root->appendChild($node);
        }
        if (isset($array->itunes->keywords)) {
            $node = $this->_element->createElementNS(
            'http://www.itunes.com/DTDs/Podcast-1.0.dtd', 'itunes:keywords', 
            $array->itunes->keywords);
            $root->appendChild($node);
        }
        if (isset($array->itunes->new_feed_url)) {
            $node = $this->_element->createElementNS(
            'http://www.itunes.com/DTDs/Podcast-1.0.dtd', 'itunes:new-feed-url', 
        ...
        if (isset($array->itunes->category)) {
            foreach ($array->itunes->category as $i => $category) {
                $node = $this->_element->createElementNS(
                'http://www.itunes.com/DTDs/Podcast-1.0.dtd', 'itunes:category');
