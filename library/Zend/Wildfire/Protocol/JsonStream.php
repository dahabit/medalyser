...
class Zend_Wildfire_Protocol_JsonStream
{
    ...
    public function getPayload (Zend_Wildfire_Channel_Interface $channel)
    {
        if (! $channel instanceof Zend_Wildfire_Channel_HttpHeaders) {
            ...
            'The ' . get_class($channel) . ' channel is not supported by the ' .
        ...
        foreach ($this->_messages as $structure_uri => $plugin_messages) {
            $payload[] = array($protocol_index . '-Structure-' .
             $structure_index, $structure_uri);
            foreach ($plugin_messages as $plugin_uri => $messages) {
                $payload[] = array($protocol_index . '-Plugin-' . $plugin_index, 
                $plugin_uri);
                        ...
                        if ($part) {
                            ...
                            if (count($parts) > 2) {
                                $msg = (($i == 0) ? strlen($message) : '') . '|' .
                                 $part . '|' . (($i < count($parts) - 2) ? '\\' : '');
                            } else {
                                $msg = strlen($part) . '|' . $part . '|';
                            }
                            $payload[] = array(
                            $protocol_index . '-' . $structure_index . '-' .
                             $plugin_index . '-' . $message_index, $msg);
                            $message_index ++;
