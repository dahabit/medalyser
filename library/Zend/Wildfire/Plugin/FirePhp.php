...
class Zend_Wildfire_Plugin_FirePhp implements Zend_Wildfire_Plugin_Interface
{
     ...
     * @var array
     */
    protected $_options = array('traceOffset' => 1, /* The offset in the trace which identifies the source of the message */
    ...
    public static function send ($var, $label = null, $style = null, $options = array())
    {
        ...
        if ($var instanceof Exception) {
            ...
            'Message' => $var->getMessage(), 'File' => $var->getFile(), 
            'Line' => $var->getLine(), 'Type' => 'throw', 
            ...
            if ($meta['Type'] == self::TRACE) {
                ...
                'Type' => $trace[0]['type'], 'Function' => $trace[0]['function'], 
                'Message' => $label, 
        ...
        if ($meta['Type'] == self::DUMP) {
            ...
            array('key' => $meta['Label'], 'data' => $var), $skipFinalEncode);
        } else {
            return $firephp->_recordMessage(self::STRUCTURE_URI_FIREBUGCONSOLE, 
            array('data' => $var, 'meta' => $meta), $skipFinalEncode);
        }
    }
    /**
     * Gets a stack trace
    ...
    protected function _recordMessage ($structure, $data, $skipEncode = false)
    {
        switch ($structure) {
            case self::STRUCTURE_URI_DUMP:
                ...
                $this, $structure, array($data['key'] => $value));
        ...
        if (is_resource($object)) {
            ...
            if (is_object($object)) {
                     ...
                     in_array($just_name, $this->_objectFilters[$class]))) {
                        if (array_key_exists($raw_name, $members) &&
                         ! $property->isStatic()) {
                            ...
                            $members[$raw_name], $objectDepth + 1, 1);
                        } else {
                            if (method_exists($property, 'setAccessible')) {
                                $property->setAccessible(true);
                                $return[$name] = $this->_encodeObject(
                                $property->getValue($object), $objectDepth + 1, 
                                1);
                            } else 
                                if ($property->isPublic()) {
                                    $return[$name] = $this->_encodeObject(
                    ...
                    if (! isset($properties[$name])) {
                        ...
                        if (! (isset($this->objectFilters[$class]) &&
                         ...
                         in_array($just_name, $this->objectFilters[$class]))) {
                            $return[$name] = $this->_encodeObject($value, 
                            $objectDepth + 1, 1);
                        } else {
                            $return[$name] = '** Excluded by Filter **';
                        }
                ...
                foreach ($object as $key => $val) {
                    ...
                    if ($key ==
                     'GLOBALS' && is_array($val) &&
                     array_key_exists('GLOBALS', $val)) {
                        $val['GLOBALS'] = '** Recursion (GLOBALS) **';
                    }
                    $return[$key] = $this->_encodeObject($val, 1, 
                    $arrayDepth + 1);
                }
            } else {
                return $object;
    ...
    public function flushMessages ($protocolUri)
    {
        ...
        foreach ($this->_messages as $message) {
            if (! $message->getDestroy()) {
                $this->send($message->getMessage(), $message->getLabel(), 
                $message->getStyle(), $message->getOptions());
            }
        }
        $this->_messages = array();
    }
