...
class Zend_Text_Table_Column
{
     ...
     * @var array
     */
    protected $_allowedAligns = array(self::ALIGN_LEFT, self::ALIGN_CENTER, 
    self::ALIGN_RIGHT);
    /**
    ...
    public function setContent ($content, $charset = null)
    {
        ...
        if ($inputCharset !== $outputCharset) {
            if (PHP_OS !== 'AIX') {
                ...
                $content = iconv($inputCharset, $outputCharset, 
                $content);
            }
        }
    ...
    public function render ($columnWidth, $padding = 0)
    {
        ...
        foreach ($lines as $line) {
            $paddedLines[] = str_repeat(' ', $padding) .
             Zend_Text_MultiByte::strPad($line, $columnWidth, ' ', $padMode, 
            $outputCharset) . str_repeat(' ', $padding);
        }
        $result = implode("\n", $paddedLines);
        return $result;
    }
