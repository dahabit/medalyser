...
class Zend_Filter_ImageSize implements Zend_Filter_Interface
{
    ...
    public function filter ($value)
    {
        if (! extension_loaded('gd')) {
            ...
            'GD extension is not available. Can\'t process image.');
        }
        ...
        if (file_exists($outputPath)) {
            switch ($this->_overwriteMode) {
                ...
                case self::OVERWRITE_NONE:
                    throw new Zend_Filter_Exception(
                    'Can\'t create thumbnail. File already exists: ' .
                     $outputPath);
                default:
                    break;
            }
    ...
    protected function _resize ($filename)
    {
        ...
        $resized = $this->getStrategy()->resize($image, $this->getWidth(), 
        $this->getHeight());
        return $resized;
    }
    /**
    ...
    Zend_Filter_ImageSize_Strategy_Interface $strategy)
    {
    ...
    {
        return $this->getThumbnailDirectory() . DIRECTORY_SEPARATOR .
    ...
    {
        if ((! in_array($mode, 
        array(self::OVERWRITE_ALL, self::OVERWRITE_NONE, 
        self::OVERWRITE_CACHE_OLDER)))) {
            throw new Zend_Filter_Exception(
            'Unsupported overwrite mode: ' . $mode);
        }
