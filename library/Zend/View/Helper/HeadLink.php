...
class Zend_View_Helper_HeadLink extends Zend_View_Helper_Placeholder_Container_Standalone
{
     ...
     * @var array
     */
    protected $_itemKeys = array('charset', 'href', 'hreflang', 'id', 'media', 
    'rel', 'rev', 'type', 'title', 'extras');
    /**
        ...
        foreach ($this->_itemKeys as $itemKey) {
            if (isset($attributes[$itemKey])) {
                if (is_array($attributes[$itemKey])) {
                    ...
                    ($this->_autoEscape) ? $this->_escape($attributes[$itemKey]) : $attributes[$itemKey]);
                }
            }
        }
    ...
    public function toString ($indent = null)
    {
        ...
        return $indent .
         implode($this->_escape($this->getSeparator()) . $indent, $items);
    }
    /**
    ...
    public function createDataStylesheet (array $args)
    {
        ...
        if (0 < count($args)) {
            ...
            if (! empty($conditionalStylesheet) &&
             is_string($conditionalStylesheet)) {
