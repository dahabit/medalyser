...
class Zend_View_Helper_Navigation_Sitemap extends Zend_View_Helper_Navigation_HelperAbstract
{
    ...
    protected function _xmlEscape ($string)
    {
        ...
        if (version_compare(PHP_VERSION, '5.2.3', '>=')) {
            ...
            return htmlspecialchars($string, ENT_QUOTES, $enc, 
            false);
        } else {
    ...
    public function getDomSitemap (Zend_Navigation_Container $container = null)
    {
        ...
        foreach ($iterator as $page) {
            ...
            $urlNode = $dom->createElementNS(self::SITEMAP_NS, 
            'url');
            ...
            $url)) {
                ...
                sprintf('Encountered an invalid URL for Sitemap XML: "%s"', 
                $url));
                $e->setView($this->view);
                throw $e;
            }
        ...
        if ($this->getUseSchemaValidation()) {
            if (! @$dom->schemaValidate(self::SITEMAP_XSD)) {
                require_once 'Zend/View/Exception.php';
                $e = new Zend_View_Exception(
                sprintf('Sitemap is invalid according to XML Schema at "%s"', 
                self::SITEMAP_XSD));
                $e->setView($this->view);
                throw $e;
            }
