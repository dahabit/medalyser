...
class Zend_Serializer_Adapter_PythonPickle extends Zend_Serializer_Adapter_AdapterAbstract
{
    ...
    protected function _write ($value)
    {
        if ($value === null) {
            ...
            require_once 'Zend/Serializer/Exception.php';
            throw new Zend_Serializer_Exception(
            'PHP-Type "' . gettype($value) . '" isn\'t serializable with ' .
             get_class($this));
        }
    }
    /**
        ...
        if ($this->_binary) {
            if ($value >= 0) {
                if ($value <= 0xff) {
                    ...
                    $this->_pickle .= self::OP_BININT1 .
                     chr($value);
                } elseif ($value <= 0xffff) {
                    // self.write("%c%c%c" % (BININT2, obj&0xff, obj>>8))
                    $this->_pickle .= self::OP_BININT2 .
                     pack('v', $value);
                }
                return;
            }
    ...
    protected function _writeString ($value)
    {
        ...
        if ($this->_binary) {
            ...
            if ($n <= 0xff) {
                ...
                $this->_pickle .= self::OP_SHORT_BINSTRING .
                 chr($n) . $value;
            } else {
    ...
    protected function _loadUnicode ()
    {
        ...
        array($this, '_convertMatchingUnicodeSequence2Utf8'), $data);
        if (self::$_isPhp6) {
    ...
    protected function _hex2Utf8 ($hex)
    {
        ...
        if ($uniCode < 0x80) { // 1Byte
            ...
            $utf8Char = chr(0xE0 | $uniCode >> 12) .
             chr(0x80 | $uniCode >> 6 & 0x3F) . chr(0x80 | $uniCode & 0x3F);
        } elseif ($uniCode < 0x110000) { // 4Byte
            $utf8Char = chr(0xF0 | $uniCode >> 18) .
             chr(0x80 | $uniCode >> 12 & 0x3F) . chr(
            0x80 | $uniCode >> 6 & 0x3F) . chr(0x80 | $uniCode & 0x3F);
        } else {
            require_once 'Zend/Serializer/Exception.php';
            throw new Zend_Serializer_Exception(
