...
class Inline
{
    ...
    {
        $trueValues = '1.1' == Yaml::getSpecVersion() ? array('true', 'on', '+', 
        'yes', 'y') : array('true');
        $falseValues = '1.1' == Yaml::getSpecVersion() ? array('false', 'off', 
        '-', 'no', 'n') : array('false');
        switch (true) {
    ...
    $stringDelimiters = array('"', "'"), &$i = 0, $evaluate = true)
    {
        if (in_array($scalar[$i], $stringDelimiters)) {
            ...
            if (! $delimiters) {
                ...
                if (preg_match('/^(.+?)(' . implode('|', $delimiters) . ')/', 
    ...
    {
        if (! preg_match('/' . self::REGEX_QUOTED_STRING . '/A', 
        substr($scalar, $i), $match)) {
            throw new ParserException(
        ...
        while ($i < $len) {
            ...
            while ($i < $len) {
                switch ($mapping[$i]) {
                    case '[':
                        // nested sequence
                        $output[$key] = self::parseSequence(
                        $mapping, $i);
                        $done = true;
                        break;
                    case '{':
                        // nested mapping
                        $output[$key] = self::parseMapping(
                        $mapping, $i);
                        $done = true;
                        break;
                    case ':':
    ...
    static protected function evaluateScalar ($scalar)
    {
        ...
        switch (true) {
            ...
            case ctype_digit($scalar):
                $raw = $scalar;
                $cast = intval($scalar);
                return '0' == $scalar[0] ? octdec($scalar) : (((string) $raw ==
                 (string) $cast) ? $cast : $raw);
            case in_array(strtolower($scalar), $trueValues):
                return true;
            case in_array(strtolower($scalar), $falseValues):
                return false;
            case is_numeric($scalar):
                return '0x' == $scalar[0] . $scalar[1] ? hexdec($scalar) : floatval(
                $scalar);
