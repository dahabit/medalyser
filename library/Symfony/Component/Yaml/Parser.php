...
class Parser
{
    ...
    public function parse ($value)
    {
        ...
        while ($this->moveToNextLine()) {
            ...
            if (preg_match('#^\t+#', $this->currentLine)) {
                ...
                sprintf(
                'A YAML file cannot contain tabs as indentation at line %d (%s).', 
            ...
            $this->currentLine, $values)) {
                 ...
                 0 === strpos(ltrim($values['value'], ' '), '#')) {
                    ...
                    $values['value'], $matches)) {
                        ...
                        if (! $this->isNextLineIndented()) {
                            ...
                            $this->getCurrentLineIndentation() + 2);
                        }
                ...
                if (preg_match(
                '#^(?P<key>' . Inline::REGEX_QUOTED_STRING .
                 '|[^ \'"].*?) *\:(\s+(?P<value>.+?))?\s*$#', $this->currentLine, 
                $values)) {
                    $key = Inline::parseScalar($values['key']);
                    if ('<<' === $key) {
                        if (isset($values['value']) &&
                         '*' === substr($values['value'], 0, 1)) {
                            $isInPlace = substr($values['value'], 1);
                            if (! array_key_exists($isInPlace, $this->refs)) {
                                throw new ParserException(
                                sprintf(
                                'Reference "%s" does not exist at line %s (%s).', 
                                $isInPlace, $this->getRealCurrentLineNb() + 1, 
                                $this->currentLine));
                            }
                        } else {
                            if (isset($values['value']) &&
                            ...
                            if (! is_array($parsed)) {
                                throw new ParserException(
                                sprintf(
                                "YAML merge keys used with a scalar value instead of an array at line %s (%s)", 
                                ...
                                if (isset($parsed[0])) {
                                    ...
                                    $parsed) as $parsedItem) {
                                        if (! is_array($parsedItem)) {
                                            throw new ParserException(
                                            sprintf(
                                            "Merge items must be arrays at line %s (%s).", 
                                            $this->getRealCurrentLineNb() + 1, 
                                            $parsedItem));
                                        }
                                        $merged = array_merge($parsedItem, 
                                        $merged);
                                    }
                                } else {
                        ...
                        if (! isset($values['value']) ||
                         ...
                         0 === strpos(ltrim($values['value'], ' '), '#')) {
                            ...
                            if ($isInPlace) {
                                ...
                                $data[$key] = $this->parseValue(
                                $values['value']);
                            }
                        }
                } else {
                    // 1-liner followed by newline
                    if (2 == count($this->lines) &&
                     empty($this->lines[1])) {
    ...
    protected function getNextEmbedBlock ($indentation = null)
    {
        ...
        if (null === $indentation) {
            ...
            if (! $this->isCurrentLineEmpty() && 0 == $newIndent) {
                throw new ParserException(
                sprintf('Indentation problem at line %d (%s)', 
                $this->getRealCurrentLineNb() + 1, $this->currentLine));
            }
        } else {
    ...
    protected function parseValue ($value)
    {
        if ('*' === substr($value, 0, 1)) {
            ...
            if (! array_key_exists($value, $this->refs)) {
                throw new ParserException(
                sprintf('Reference "%s" does not exist (%s).', $value, 
                $this->currentLine));
            }
        ...
        $value, $matches)) {
            ...
            return $this->parseFoldedScalar($matches['separator'], 
    ...
    $indentation = 0)
    {
        ...
        if (! preg_match(
        '#^(?P<indent>' . ($indentation ? str_repeat(' ', $indentation) : ' +') .
        ...
        while ($this->currentLineNb + 1 < count($this->lines)) {
            $this->moveToNextLine();
            if (preg_match(
            '#^(?P<indent> {' . strlen($textIndent) . ',})(?P<text>.+)$#', 
            $this->currentLine, $matches)) {
                ...
                $text .= str_repeat(' ', 
                $diff = strlen($matches['indent']) - strlen($textIndent)) .
                 $matches['text'] . ($diff ? "\n" : $separator);
            } else 
