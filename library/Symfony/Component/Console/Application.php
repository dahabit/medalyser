...
class Application
{
    ...
    public function __construct ($name = 'UNKNOWN', $version = 'UNKNOWN')
    {
        ...
        new InputOption('--ansi', '-a', InputOption::VALUE_NONE, 
        'Force ANSI output.'), 
    ...
    public function findNamespace ($namespace)
    {
        ...
        if (! isset($abbrevs[$namespace])) {
            throw new \InvalidArgumentException(
            sprintf('There are no commands defined in the "%s" namespace.', 
            $namespace));
        }
        if (count($abbrevs[$namespace]) > 1) {
            throw new \InvalidArgumentException(
            sprintf('The namespace "%s" is ambiguous (%s).', $namespace, 
            $this->getAbbreviationSuggestions($abbrevs[$namespace])));
        }
    ...
    public function find ($name)
    {
        ...
        if (isset($abbrevs[$name]) && 1 == count($abbrevs[$name])) {
            return $this->get(
            $namespace ? $namespace . ':' . $abbrevs[$name][0] : $abbrevs[$name][0]);
        }
        if (isset($abbrevs[$name]) && count($abbrevs[$name]) > 1) {
            ...
            throw new \InvalidArgumentException(
            sprintf('Command "%s" is ambiguous (%s).', $fullName, $suggestions));
        }
        // aliases
        $abbrevs = static::getAbbreviations(array_keys($this->aliases));
        if (! isset($abbrevs[$fullName])) {
            throw new \InvalidArgumentException(
            sprintf('Command "%s" is not defined.', $fullName));
        }
        if (count($abbrevs[$fullName]) > 1) {
            throw new \InvalidArgumentException(
            sprintf('Command "%s" is ambiguous (%s).', $fullName, 
            $this->getAbbreviationSuggestions($abbrevs[$fullName])));
        }
    ...
    public function asText ($namespace = null)
    {
        ...
        foreach ($this->sortCommands($commands) as $space => $commands) {
            ...
            foreach ($commands as $command) {
                $aliases = $command->getAliases() ? '<comment> (' .
                 implode(', ', $command->getAliases()) . ')</comment>' : '';
                $messages[] = sprintf("  <info>%-${width}s</info> %s%s", 
                ($command->getNamespace() ? ':' : '') . $command->getName(), 
        ...
        foreach ($this->sortCommands($commands) as $space => $commands) {
            ...
            foreach ($commands as $command) {
                if (! $namespace) {
                    ...
                    $dom->createTextNode($command->getName()));
                }
                $node = $command->asXml(true)
    ...
    public function renderException ($e, $output)
    {
        ...
        {
            return function_exists('mb_strlen') ? mb_strlen($string) : strlen(
            $string);
        };
        do {
            ...
            if (Output::VERBOSITY_VERBOSE === $output->getVerbosity()) {
                ...
                for ($i = 0, $count = count($trace); $i < $count; $i ++) {
                    ...
                    $output->writeln(
                    sprintf(' %s%s%s() at <info>%s:%s</info>', $class, $type, 
                    $function, $file, $line));
                }
                $output->writeln("\n");
            }
